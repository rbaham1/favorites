...
/// Enables control over a single [FavoritesExpansionTile]'s expanded/collapsed state.
///
/// It can be useful to expand or collapse an [FavoritesExpansionTile]
/// programmatically, for example to reconfigure an existing expansion
/// tile based on a system event. To do so, create an [FavoritesExpansionTile]
...
/// `build` method of a descendant of an [FavoritesExpansionTile].
///
/// When the [FavoritesExpansionTile] is actually created in the same `build`
...
/// [BuildContext] that is "under" the [FavoritesExpansionTile]:
///
...
/// would have an outer widget that creates the [FavoritesExpansionTile]
...
/// methods cause the [FavoritesExpansionTile] to rebuild, so they may not be called from
/// a build method.
///
...
/// [FavoritesExpansionTile] to save and restore its expanded state when it is scrolled
/// in and out of view.
///
...
/// to the [leading] and [trailing] properties of [FavoritesExpansionTile].
///
/// {@tool dartpad}
/// This example demonstrates how the [FavoritesExpansionTile] icon's location and appearance
/// can be customized.
///
...
/// programmatically expand or collapse an [FavoritesExpansionTile].
///
/// ** See code in examples/api/lib/material/expansion_tile/expansion_tile.1.dart **
/// {@end-tool}
///
/// ## Accessibility
///
/// The accessibility behavior of [FavoritesExpansionTile] is platform adaptive, based on
...
class FavoritesExpansionTile extends StatefulWidget {
  ...
  /// Specifies if the [FavoritesExpansionTile] should build a default trailing icon if [trailing] is null.
  ...
  ///
  /// {@tool dartpad}
  /// This sample showcases how to override the [FavoritesExpansionTile] expansion
  ...
  @override
  State<FavoritesExpansionTile> createState() => _ExpansionTileState();
}

class _ExpansionTileState extends State<FavoritesExpansionTile> {
    ...
    final Widget child = ListTileTheme.merge(
      ...
      child: Material(
        ...
        child: ListTile(
            ...
            trailing: _buildTrailingIcon(context, animation),
  ...
  @override
  void didUpdateWidget(covariant FavoritesExpansionTile oldWidget) {
...
// BEGIN GENERATED TOKEN PROPERTIES - FavoritesExpansionTile
...
// END GENERATED TOKEN PROPERTIES - FavoritesExpansionTile
